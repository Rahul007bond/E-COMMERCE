...
if(isset($_POST['getdata'])||isset($_POST['get_seleted_Category'])||isset($_POST['getcheck'])){
  
    if(isset($_POST['get_seleted_Category'])){  $search=$_POST['search'];
        $cat=$_POST['cat_id'];
        $sql="select * from uproducts where productkeyword like '%$search%' or product_title like '%$search%' and product_catid='$cat' ";
    }
    ...
    if(isset($_POST['avail'])){
        $sql="select * from uproducts where available > 0 ";
      }
  
  if(isset($_POST['avail'])&&isset($_POST['s'])){
    $sql="select * from uproducts where productkeyword like '%$s%' or product_title like '%$s%' and available > 0 ";
  }
    ...
    $limit=$_POST['limit'];
 if($search==""){
    $sql="select * from uproducts limit  $start,$limit ";
 }else
$sql="select * from uproducts where productkeyword like '%$search%' or product_title like '%$search%' limit  $start,$limit ";}
...
<div class='card-footer d-flex justify-content-between border'>
   <a href='uproducts.php?pro_id=$id' class='btn btn-lg'><i class='fas fa-eye  mr-1'></i>View Detail</a>
...
";}
    ...
    echo "<div class='col-lg-4 col-md-6 col-sm-6  pb-1' style='pointer-events: none; opacity:0.5;'>
<div class='card product-item border-0 mb-4'>
<div class='card-header product-img position-relative overflow-hidden bg-transparent border p-0'>
   <img class='img-fluid w-100' src='img/$imgp' alt=''>
   <span style='padding:3px;font-size:13px;color:white; margin:1px; background-color:#388e3c; ' >$rsd <i class='fas fa-star'></i> </span>				
